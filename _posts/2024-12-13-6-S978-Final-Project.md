...
At its core, FID quantifies the similarity between the real data distribution and the distribution of samples generated by a model. It achieves this as follows. Let $\\{x_i\\}^n_{i=1}$ represent the real data and {% raw %}$\\{x_j\\}^{n}_{j=1}${% end raw %} the model-generated samples. Using a high-fidelity feature extractor $f$, such as the Inception v3 model pretrained on ImageNet [15, 16], the features of $x_i$ and $\tilde{x}_j$ are obtained as:
\\[f_i = f(x_i) \quad \text{and} \quad \tilde{f}_j = f(\tilde{x}_j), \quad \forall \, i, \, j.\\]
...
\\[
\mu_g = \frac{1}{\tilde n} \sum_{j=1}^{\tilde n} \tilde{f}_j, \quad \Sigma_g = \frac{1}{\tilde{n}} \sum_{j=1}^{\tilde{n}} (\tilde{f}_j - \mu_g)(\tilde{f}_j - \mu_g)^T.
\\]
