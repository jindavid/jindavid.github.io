...
At its core, FID quantifies the similarity between the real data distribution and the distribution of samples generated by a model. It achieves this as follows. Let $\\{x_i\\}^n_{i=1}$ represent the real data and \\(\\{x_j\\}^{n}_{j=1}\\) the model-generated samples. Using a high-fidelity feature extractor $f$, such as the Inception v3 model pretrained on ImageNet [15, 16], the features of $x_i$ and $\tilde{x}_j$ are obtained as:
