...
At its core, FID quantifies the similarity between the real data distribution and the distribution of samples generated by a model. It achieves this as follows. Let $\\{x_i\\}^n_{i=1}$ represent the real data and $\\{y_i\\}^n_{i=1}$ the model-generated samples. Using a high-fidelity feature extractor $f$, such as the Inception v3 model pretrained on ImageNet [15, 16], the features of $x_i$ and $\y_j$ are obtained as:
...
\\[\mu_g = \frac{1}{\tilde n} \sum_{j=1}^{\tilde n} g_j, \quad \Sigma_g = \frac{1}{m} \sum_{j=1}^{m} (g_j - \mu_g)(g_j - \mu_g)^T.\\]
